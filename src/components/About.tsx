...
export function About() {
  ...
  return (
    <section
      id="about"
      className="relative z-[1] bg-white px-5 py-20 sm:px-8 sm:py-24 lg:min-h-screen lg:px-12 lg:py-32"
    >
      <div className="mx-auto grid w-full max-w-shell items-center gap-14 lg:grid-cols-2 lg:gap-20">
        <Reveal className="relative h-[420px] sm:h-[520px] lg:h-[600px]">
          <div className="absolute left-1/2 top-1/2 h-[360px] w-[280px] -translate-x-1/2 -translate-y-1/2 bg-gradient-to-br from-metal-light to-metal-mid clip-octagon-soft sm:h-[450px] sm:w-[350px]" />
          <motion.div
            className="absolute right-[7%] top-[8%] h-24 w-24 rounded-full border-2 border-metal-mid sm:right-[10%] sm:top-[10%] sm:h-[120px] sm:w-[120px]"
            animate={shouldReduceMotion ? undefined : { scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
            transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute bottom-[15%] left-[4%] h-20 w-20 bg-black clip-pentagon"
        ...
        <Reveal className="lg:pr-12">
          ...
          <div className="space-y-6 text-base font-light leading-8 text-metal-dark sm:text-lg">
            <p>
              MetalFlux was born from a desire to break free from conventional fashion. We believe
          ...
          <div className="mt-10 grid gap-5 border-t border-metal-light pt-10 sm:mt-12 sm:grid-cols-3 sm:gap-7 sm:pt-12">
            {stats.map((stat) => (
              <div key={stat.label}>
                <div className="mb-2 font-display text-4xl font-bold text-black sm:text-[2.5rem]">
                  {stat.value}
                </div>
